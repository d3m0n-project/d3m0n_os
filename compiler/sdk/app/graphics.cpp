		...
		for (dy = 0; dy < dst_h; dy++)
		{
			...
			for (dx = 0; dx < dst_w; dx++)
			{
				...
				if (data & (1U << ((8 * bytes_per_line - 1) - src_x)))
					this->put_pixel(cursor_x + dx, cursor_y + dy, color);
			}
		}
		cursor_x += dst_w;
		i++;
...
static void svg_points(Display *display, const char *tag, int length, int ox, int oy, int sx, int sy, uint32_t color, bool close)
{
	...
	{
		while (points < tag + length && (*points < '0' || *points > '9') && *points != '-') ++points;
		if (points >= tag + length) break;
		int nx = svg_attr_int(points, (int)(tag + length - points), "", 0);
		while (points < tag + length && *points != ',' && *points != ' ' && *points != '\t') ++points;
		while (points < tag + length && (*points == ',' || *points == ' ' || *points == '\t')) ++points;
		int ny = svg_attr_int(points, (int)(tag + length - points), "", 0);
		while (points < tag + length && *points != ',' && *points != ' ' && *points != '\t') ++points;
		int dx = ox + nx * sx / 1000;
		int dy = oy + ny * sy / 1000;
		if (have) svg_line(display, px, py, dx, dy, color);
		else { first_x = dx; first_y = dy; have = true; }
		px = dx;
		py = dy;
	}
	if (close && have) svg_line(display, px, py, first_x, first_y, color);
}
...
static void svg_path(Display *display, const char *tag, int length, int ox, int oy, int sx, int sy, uint32_t color, bool filled)
{
	...
	while (cursor < end && *cursor != '"' && *cursor != '\'')
	{
		...
		int found = 0;
		while (found < needed && svg_next_number(&cursor, end, &values[found])) ++found;
		if (found != needed) { ++cursor; continue; }
		int next_x = current_x, next_y = current_y;
		if (command == 'H' || command == 'h') next_x = command == 'h' ? current_x + values[0] : values[0];
		else if (command == 'V' || command == 'v') next_y = command == 'v' ? current_y + values[0] : values[0];
		else if (command == 'C' || command == 'c') { next_x = command == 'c' ? current_x + values[4] : values[4]; next_y = command == 'c' ? current_y + values[5] : values[5]; }
		else if (command == 'S' || command == 's' || command == 'Q' || command == 'q') { next_x = command == 's' || command == 'q' ? current_x + values[2] : values[2]; next_y = command == 's' || command == 'q' ? current_y + values[3] : values[3]; }
		else { next_x = command == 'm' || command == 'l' ? current_x + values[0] : values[0]; next_y = command == 'm' || command == 'l' ? current_y + values[1] : values[1]; }
		int screen_x = ox + next_x * sx / 1000;
		int screen_y = oy + next_y * sy / 1000;
		if (count > 0) svg_line(display, points_x[count - 1], points_y[count - 1], screen_x, screen_y, color);
		if (count < 256) { points_x[count] = screen_x; points_y[count++] = screen_y; }
		current_x = next_x;
		current_y = next_y;
		if (command == 'M') { start_x = current_x; start_y = current_y; command = 'L'; }
		if (command == 'm') { start_x = current_x; start_y = current_y; command = 'l'; }
	}
	if (filled && count > 2)
	{
		for (int y = 0; y < display->h; ++y)
		{
			...
			for (int i = 0, j = count - 1; i < count; j = i++)
				if ((points_y[i] > y) != (points_y[j] > y)) intersections[hits++] = points_x[i] + (y - points_y[i]) * (points_x[j] - points_x[i]) / (points_y[j] - points_y[i]);
			for (int i = 0; i + 1 < hits; i += 2) display->draw_hline(intersections[i], y, intersections[i + 1] - intersections[i] + 1, color);
		}
	}
}
...
void Display::draw_svg(int x, int y, int w, int h, const char *path, uint32_t override_color)
{
	...
	if (source_width <= 0) source_width = 320;
	if (source_height <= 0) source_height = 480;
	...
	while (cursor < svg + total)
	{
		...
		if (strnstr(begin, "<rect", length)) draw_rect(px, py, pw, ph, color);
		else if (strnstr(begin, "<circle", length)) draw_ellipse(x + svg_attr_int(begin, length, "cx", 0) * w / source_width, y + svg_attr_int(begin, length, "cy", 0) * h / source_height, svg_attr_int(begin, length, "r", 0) * w / source_width, svg_attr_int(begin, length, "r", 0) * h / source_height, color, 1);
		else if (strnstr(begin, "<ellipse", length)) draw_ellipse(x + svg_attr_int(begin, length, "cx", 0) * w / source_width, y + svg_attr_int(begin, length, "cy", 0) * h / source_height, svg_attr_int(begin, length, "rx", 0) * w / source_width, svg_attr_int(begin, length, "ry", 0) * h / source_height, color, 1);
		else if (strnstr(begin, "<line", length)) svg_line(this, x + svg_attr_int(begin, length, "x1", 0) * w / source_width, y + svg_attr_int(begin, length, "y1", 0) * h / source_height, x + svg_attr_int(begin, length, "x2", 0) * w / source_width, y + svg_attr_int(begin, length, "y2", 0) * h / source_height, color);
		else if (strnstr(begin, "<polygon", length)) svg_points(this, begin, length, x, y, w * 1000 / source_width, h * 1000 / source_height, color, true);
		else if (strnstr(begin, "<polyline", length)) svg_points(this, begin, length, x, y, w * 1000 / source_width, h * 1000 / source_height, color, false);
		else if (strnstr(begin, "<path", length)) svg_path(this, begin, length, x, y, w * 1000 / source_width, h * 1000 / source_height, color, true);
		cursor = end + 1;
	}
	delete[] svg;
}

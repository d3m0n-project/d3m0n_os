...
Control* loadControl(display* display1, string controlName, string controlAttributes) {
    Control* controlElement = nullptr;

    if(controlName == "Window") {
		layout::Window(display1, controlName, controlAttributes);
	}
	else if(controlName=="Text") {
        layout::Text* temp = new layout::Text(controlAttributes.c_str());
		temp->display1 = display1;

...
public:
    ...
    void runApp(display* display1, Application* app)
    {
        ...
        layoutHandler::runLayout(display1, app, app->temp_path+(string)"/layouts/main.layout");
        sourceHandler::runSource(display1, app, app->temp_path+(string)"/src/main.src");
    ...
    void mainScreenInit(display* display1) {
        printf("\n");

        // init main desktop screen
        string wallpaper = GetPath()+"/wallpapers/"+getSetting("wallpaper", (char*)GetConfig())+".png";
        display1->drawImage(DataTypes::Point(0, 0), wallpaper, SCREEN_WIDTH, SCREEN_HEIGHT, false);
        log("DESKTOP initiated wallpaper => '"+wallpaper+"'", LogStatus::Success);
    
        logn("Loaded applications: ", ConsoleColor::Cyan);
        ...
        for (const auto & entry : fs::directory_iterator(GetAppPath())) {
            if(endsWith(entry.path(), ".d3m0n")) {
                ...
                logn("      => "+(string)entry.path(), ConsoleColor::Cyan);
                printf("      ");
                temp = loadApp(((string)entry.path()).data());
                logn("      x: "+to_string(slot_x)+", y: "+to_string(slot_y), ConsoleColor::Grey);
                
                if(contains(temp.icon, "/")) {
                    appicon=temp.icon;
                } else {
                    appicon=display1->GetThemeIcon(temp.icon);
                }
                ...
                display1->drawImage(appRect+10, appicon, icon_size-20, icon_size-20, false);
                display1->drawText(20+slot_x*(icon_size+app_padding), slot_y*(icon_size+app_padding)+icon_size-8, 1, temp.name, false, Colors::White);

                display1->registerEvent(Event::OnClick, DataTypes::Rect(appRect.X, appRect.Y, appRect.X+icon_size, appRect.Y+icon_size+20), [this](Application* app) { this->launchApp(app); }, &temp);

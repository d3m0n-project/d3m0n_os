...
vector<UIEvent::Listener> display::events;

void display::setColorFromRGB(int r, int g, int b) {
    glColor3f(r / 255.0f, g / 255.0f, b / 255.0f);
}

// GLFWwindow* window;
// vector<UIEvent::Listener> events;
void display::registerEvent(short type, DataTypes::Rect rect, function<void(Application)> function, Application app) {
    log("Registering new event id:"+to_string(type), LogStatus::Loading);
    
    UIEvent::Listener listener;
    listener.type = type;
    listener.rect = rect;
    ...
    if(isDebugMode()) {
        ...
        while(true) { 
            ...
            if (state == GLFW_PRESS && !clicking)
            {
                ...
                for(UIEvent::Listener event: events) {
                    if(event.type == UIEvent::OnClick) {
                        if(display::checkCollision(DataTypes::Point((int)xpos, (int)ypos), event.rect)) {
                            // event.callback(event.app);
                            appManager::launchApp(event.app);
                        }
                    }
                }
...
bool display::checkCollision(DataTypes::Point point, DataTypes::Rect rect) {
...
void display::PlaySound(string name)
	{
		...
		try
		{
			...
			{
                system(("paplay '"+theme_path+"/"+name+".ogg'").c_str());
...
string display::GetThemeIcon(string name) {
...
string display::displayCommand(string command) {
    string output="";
...
// Drawing functions
void display::drawCircle(int x1, int y1, int radius, ushort color, bool fill) {
...
// round rect
void display::drawRoundRect(int x1, int y1, int x2, int y2, ushort color, int radius, bool fill) 
    {
...
// rect
void display::drawRect(int x1, int y1, int x2, int y2, ushort color, bool fill)
{
        ...
        if(fill) {
            ...
            for(int y=0; y<height; y++) {
                for(int x=0; x<width; x++) {
                    display::drawPixel(x1+x, y1+y, color);
                }
            }
            drawScreen();
        } else {
...
// line
void display::drawLine(int x1, int y1, int x2, int y2, ushort color) 
{
    if(isDebugMode()) {
        glBegin(GL_LINES);
        uint8_t r,g,b;
        RGB565toRGB(color, r, g, b);
        display::setColorFromRGB(r,g,b);
        glVertex2f(x1, y1);
        glVertex2f(x2, y2);
        glEnd();
        drawScreen();
...
void display::drawPixel(int x, int y, ushort color)
{
...
void display::drawText(int x, int y, int fontSize, string text, bool bold, ushort color)
{
...
// image draw
void display::drawImage(DataTypes::Point location, string file, int width, int height, bool reverse) {

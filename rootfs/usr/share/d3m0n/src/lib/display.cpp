...
class display {
    ...
    void drawCircle(int x1, int y1, int radius, ushort color, bool fill=false) {
        if(isDebugMode()) {
            ...
            circle(image, Point(x1, y1), radius, Scalar((int)r, (int)g, (int)b), isFilled);
            // Display the image in the window
            imshow("d3m0n emulator", image);
        } else {
            if (fill) {
    ...
    void drawRect(int x1, int y1, int x2, int y2, ushort color, bool fill=false)
    {
        ...
        if(isDebugMode()) {
            ...
            log(to_string(color), LogStatus::Info);

            if(fill) { isfilled=-1; }
            rectangle(image, Point(x1, y1), Point(x2, y2), Scalar((int)r, (int)g, (int)b), isfilled);
            // Display the image in the window
            imshow("d3m0n emulator", image);
        } else {
            if(fill) {
    ...
    void drawLine(int x1, int y1, int x2, int y2, ushort color) {
        if(isDebugMode()) {
            ...
            uint8_t b;
            RGB565toRGB((uint16_t)color, r, g, b);
            line(image, Point(x1, y1), Point(x2, y2), Scalar((int)r, (int)g, (int)b), 2);
        } else {
    ...
    {
        Mat image2 = imread(file.c_str(), IMREAD_COLOR);

...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		i = d & 3;								/* Get table ID */
		pb = static_cast<uint8_t*>(alloc_pool(jd, 64 * sizeof (int32_t)));/* Allocate a memory block for the table */
...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		pb = static_cast<uint8_t*>(alloc_pool(jd, 16));			/* Allocate a memory block for the bit distribution table */
		if (!pb) return JDR_MEM1;			/* Err: not enough memory */
		jd->huffbits[num][cls] = pb;
		for (np = i = 0; i < 16; i++) {		/* Load number of patterns for 1 to 16-bit code */
			np += (pb[i] = *data++);		/* Get sum of code words for each code */
		}
		ph = static_cast<uint16_t*>(alloc_pool(jd, np * sizeof (uint16_t)));/* Allocate a memory block for the code word table */
		...
		ndata -= np;
		pd = static_cast<uint8_t*>(alloc_pool(jd, np));			/* Allocate a memory block for the decoded data */
...
)
{
	...
	jd->inbuf = seg = static_cast<uint8_t*>(alloc_pool(jd, JD_SZBUF));		/* Allocate stream input buffer */
	...
	for (;;) {				/* Parse JPEG segments */
		...
		case 0xDA:	/* SOS - Start of Scan */
			...
			jd->mcubuf = static_cast<jd_yuv_t*>(alloc_pool(jd, (n + 2) * 64 * sizeof (jd_yuv_t)));	/* Allocate MCU working buffer */

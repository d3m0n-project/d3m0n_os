	...
	for (int y = rect->top; y <= rect->bottom; y++) {
		for (int x = rect->left; x <= rect->right; x++) {
			...
			if (y < jd->screenHeight && x < jd->screenWidth) {
				jd->outData[y][x] = RGBtoRGB565(in[0], in[1], in[2]);
			}
...
int16_t decode_jpeg(pixel_jpeg ***pixels, char * file, uint16_t width, uint16_t height, uint16_t * imageWidth, uint16_t * imageHeight) {
	...
	*pixels = static_cast<pixel_jpeg**>(calloc(height, sizeof(pixel_jpeg *)));
	if (*pixels == NULL) {
	...
	for (int i = 0; i < height; i++) {
		(*pixels)[i] = static_cast<pixel_jpeg*>(malloc(width * sizeof(pixel_jpeg)));
		if ((*pixels)[i] == NULL) {
	...
	work = static_cast<char*>(calloc(WORKSZ, 1));
	if (work == NULL) {

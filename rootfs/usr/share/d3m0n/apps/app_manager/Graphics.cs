...
namespace d3m0n
{
	...
	{
		public static Dictionary<string, Window> available_windows = new Dictionary<string, Window>();
		public Window into;
		...
		public static void destroy(string package)
		{
			...
			utils.logn("[~] closing '"+package+"'", ConsoleColor.Magenta);
			try
			{
				...
				{
					utils.logn("[x] App '"+package+"' seems to be not oppened", ConsoleColor.Red);
				}
			}
			catch(Exception)
			{
				utils.logn("[x] an error occured while closing an app", ConsoleColor.Red);
			}
		...
		public static void display(string appInstance, Control control)
		{
			try {
				...
				// control.draw();
				available_windows[appInstance].Add(control);
				Label tmp = control as Label;	
				tmp.draw();
			}
			catch(Exception)
			{
		...
		public Control layout_to_form(string control, Dictionary<string, string> args, Control ctrl=null)
		{
			...
			// }
				...
				return layout.Image(args, this, ctrl as Image);
			}
				...
				return layout.RoundButton(args, this, ctrl as RoundButton);
			}
			else {

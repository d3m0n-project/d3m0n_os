...
namespace d3m0n_modules
{
    ...
    {
        public static void radio_hijack_stop()
        {
            utils.logn("[o] stopped hijacking radio waves", ConsoleColor.Green);
            execute("sudo pkill -f pi_fm_rds");
        }
        public static void radio_hijack(float freq, string soundFile, string radioName, string radioText)
        {
            ...
            if(File.Exists(utils.getPath()+"/lib/PiFmRds"))
            {
                ...
                utils.logn("    - '"+radioName+"' => '"+radioText+"'", ConsoleColor.Magenta);

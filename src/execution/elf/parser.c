...
static int	load_executable(elf_header_32 header, elf_header_identification identification_header, char *buffer)
{
	...
	for (int i=0; i<u16(header.PROGRAM_HEADER_ENTRY_COUNT); i++)
	{
		...
		log("\t- TYPE:			0x%x\n", 0, u32(program_headers[i].TYPE));
		log("\t- OFFSET:		0x%x\n", 0, u32(program_headers[i].OFFSET));
		log("\t- PHYSICAL_ADDR:	0x%p\n", 0, (uint64_t)u32(program_headers[i].PHY_ADDR));
		log("\n", 0);
		if (u32(program_headers[i].TYPE) == PT_LOAD)
		{
			//void *dest = map(program_headers[i].VIRT_ADDR, program_headers[i].MEMORYSIZE);

			//ft_memcpy(dest, buffer + program_headers[i].OFFSET, program_headers[i].FILESIZE);
			//ft_memcpy(dest + program_headers[i].FILESIZE, 0, program_headers[i].MEMORYSIZE - program_headers[i].FILESIZE);
			break;
		}
	}
...
int	parse_elf_headers(int fd)
{
	...
	if (ft_memcmp(buffer, ELF_SIG, 4) == 0)
	{
		...
		if (identification_header.BITS_ARCH == 1) // 32 bits
		{
			...
			{
				log("ELF file is not executable!\n", LOG_ERROR);
				return 0;
			}
			...
			switch (u16(header.FILE_TYPE))
			{
			case ET_EXEC:
				return load_executable(header, identification_header, buffer);
				break;

			case ET_DYN:
				//load_shared_or_pie(...); // TODO:
				break;

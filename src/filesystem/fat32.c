...
static int fat32_create_root_entry(const uint8_t name83[11], uint32_t first_cluster)
{
	uint32_t cluster;
	uint32_t entries;
	uint32_t i;
	...
	if (!name83 || first_cluster < 2 || !fat32_ready())
		return -1;
	...
	while (cluster)
	{
		...
		for (i = 0; i < entries; i++)
		{
			...
			if (entry[DIR_NAME_OFF] == 0xE5)
				continue;
			if (entry[DIR_ATTR_OFF] == 0x0F)
				continue;
			if (entry[DIR_ATTR_OFF] & 0x08)
				continue;

			if (fat32_name_match(entry + DIR_NAME_OFF, filename))
			{
				file.first_cluster =
					((uint32_t)mem_read16(entry + DIR_FSTCLUSHI_OFF) << 16)
					| mem_read16(entry + DIR_FSTCLUSLO_OFF);
				if (file.first_cluster < 2 || file.first_cluster >= fat32_max_clusters())
				{
					file.first_cluster = 0;
					return file;
				}

				file.size = mem_read32(entry + DIR_FILESIZE_OFF);
				file.pos = 0;
...
FAT32_File fat32_create(const char *filename)
{
	...
	if (fat32_open(filename).first_cluster != 0)
		return file;
	...
	if (fat32_create_root_entry(name83, new_cluster) != 0)
		return file;

	created = fat32_open(filename);
	if (created.first_cluster == new_cluster)
		return created;
